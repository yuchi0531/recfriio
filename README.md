...
Linux用Friio録画プログラム(黒凡/白凡共通)です。こちらは亜流版ですのでご注意下さい。

## 最新ツールチェーン対応

本リポジトリは最新のディストリビューション（GCC 15 / C++17 / Boost 1.87以降 / glibc 2.4x系、Ubuntu 24.04で確認）でビルドできるよう修正済みです。HDUS/HDP対応(as11loaderを除く)を含みます。
...
```sh
apt install -y build-essential libglib2.0-dev libboost-dev libboost-thread-dev libboost-filesystem-dev libpcsclite-dev
```
...
```sh
cd recfriior5
make clean   # 必須: Makefile.depが古いと差分ビルドで再コンパイルされないため
make
./recfriio   # usageが表示されればOK
```

### 対応済みの修正

- C++17で廃止された動的例外仕様 `throw (...)` を除去
- `usbdevfs_urb` のflexible array member対策（構造体に直接埋め込めないため）
- Boost 1.87+ で分離された `boost/filesystem/directory.hpp` のinclude追加
- Boost 1.90+ で統合され存在しない `-lboost_system` を除去
- arib25/Makefileのリンク順修正（ライブラリをオブジェクトの後ろに配置）
- TS Splitter用バッファ `splitbuf_t` を32MB→1MBに縮小
  （スタック上に32MB置くとスタック8MB制限で即セグフォするため。実録画では影響ありません）

## 使い方

`make` でビルドした後、以下で録画されます。
...
Makefile中の各定義をコメントアウトせずにビルドした場合、対応する機能が有効になります。

| 定義 | 機能 |
|---|---|
| `B25 = -DB25` | まるも氏のARIB STD-B25 仕様確認テストプログラムがリンクされ、B25デコード機能が使用可能になります。 |
| `HDUS = -DHDUS` | MonsterTV HDUS/HDP対応版としてコンパイルされます。 |
| `UDP = -DUDP` | UDP送信対応版としてコンパイルされます。 |
| `HTTP = -DHTTP` | HTTPサーバ機能搭載版としてコンパイルされます。 |

### B25デコード

- オプション `--b25` を指定すると、録画時にB25デコードを行います。
- `--round N` `--strip` `--EMM` 等のオプションがあり、ARIB STD-B25 仕様確認テストプログラムと同じように動作します。
- `--b25` オプションを使用しない場合、録画ファイルはMulti2で暗号化された状態で出力されるのでb25等でデコードして下さい。

## 排他ロック

recfriio r3 からFriioの初期化時に黒凡/白凡判定を行なうようになった為、flockでの排他の為にファイルを使用します。

- デフォルトでは `/var/lock/friiodetect` が使用されますが、`--lockfile ファイル名` で変更が可能です。
- 変更する場合、全てのrecfriioが同じロックファイルを使用するように注意して下さい。
- 排他用ファイルに書き込み権限が必要なので、例えばFriioを使用するユーザが全てvideoグループに属している場合、以下のように事前にファイルを作成しておいて下さい。
...
`/dev/bus/usb` の存在を確認し、なければ `/proc/bus/usb` を見に行きます。
Friioに対応するファイルに書き込み権限が必要です。

udevを使用している場合は、`/etc/udev/rules.d/91-permissions.rules` を参考にしてudevの設定ファイルを修正して下さい。ディストリビューションによりファイル名等は変わると思います。下記が追加した部分です。実行ユーザがvideoグループに属している事を前提に書いています。
...
- USBの通信がタイムアウトした場合の処理を行っていない。
  - => 多少リカバリーを行うようにしました。
    ...
    - バルク転送時のタイムアウトは無視します。実装方法に多少問題があり、タイムアウトが大量に発生した場合対応できません。（バッファオーバーフローになります。）
    - 終了時にタイムアウトした場合無視して正常終了します。LEDの色が元に戻りません。
- boost::threadのライブラリ名が-mt付きの場合と-mt無しの場合がある。
  - => コンパイルエラーが起こった場合、Makefile内にある `-lboost_thread-mt` の-mtを外して下さい。
...
- `--b25` を使用した録画中にB25でエラーが発生した場合にB25なしで録画を継続するようにしました。
...
このプログラムは以下のBonDriverからの派生物になります。

- BonDriver(Meru customize edition)
...
上記ソースからの派生部分については大元のBonDriverのライセンスである、以下が適用されます。
...
また、ARIB STD-B25 仕様確認テストプログラムをリンクした場合（デフォルトでリンクします）、BonDriverのライセンスに加えてARIB STD-B25 仕様確認テストプログラムのライセンスである、以下が適用されます。
...
ソース中、`.trace` の付いたファイルを `.trace` 無しのファイル名に変更してビルドする事によりエラー発生時のスタックトレース機能が有効になります。

このコードは http://0xcc.net/blog/archives/000073.html からの派生物となります。ビルドにはlibbfd(binutils-dev)が必要になります。
